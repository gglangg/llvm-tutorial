...
/*
// ifndef指令是「如果沒有定義」的意思，後面跟著一個巨集名稱（在這個例子中是 ADD_CONST_H）。
// 如果這個巨集在之前沒有被定義過，那麼 #ifndef 和 #endif 之間的程式碼就會被包含在這個檔案中。
*/
...
namespace addconst {
    ...
    struct AddModuleIrPrinter : public llvm::PassInfoMixin<AddModuleIrPrinter> {
        ...
        // AddPrinterClass.
        explicit AddModuleIrPrinter(llvm::raw_ostream &OS) : OS(OS) {}
