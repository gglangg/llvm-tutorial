...
void registerModuleAnalyses(ModuleAnalysisManager &MAM) {
    MAM.registerPass([] { return addconst::AddModuleAnalysis(); });
}
...
bool registerModulePipeline(StringRef Name, ModulePassManager &MPM,
                      ArrayRef<PassBuilder::PipelineElement>) {
    if (Name == "print<ir>") {
        MPM.addPass(addconst::AddModuleIrPrinter(errs()));
        return true;
    }           
    return false;     
}
